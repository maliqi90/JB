...
void TIM2_CONFIG(void)
{
	...
	TIM2->ARR = 10000;  //设定计数器自动重装值//刚好1ms    
	TIM2->PSC = SystemCoreClock / 1000000 - 1;    //预分频器,

	TIM2->CCMR2 |= 2 << 8; //CC4通道被配置为输入，IC4映射在TI3上；
	TIM2->CCER  |= 3 << 12; //配置输入/捕获4输出极性及使能CC4

	TIM2->DIER  |= 1 << 4;   //允许更新捕获中断4				
...
{	
	...
	TIM4->CR1   = (1 << 7) | (1 << 4) | (1 << 2);
	TIM4->CR2   = 0;
	TIM4->SMCR  = 0;
	TIM4->DIER  = (1 << 6) | (1 << 0);
	TIM4->CCER  = 0;
	TIM4->PSC   = SystemCoreClock / 1000000 - 1;  //分频
	TIM4->CNT   = 50000;					
	TIM4->CR1  |= 1 << 0;

	NVIC->IP[TIM4_IRQn] = 0x30;
