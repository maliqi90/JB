...
//	GPIOB->CRH |= 0X30000000;	  // Pb15 BL背光 普通IO输出
  ...
  //命令数据线 PD11  a16
	...
	GPIOD->CRL &= 0xFF00FFFF;   // PD4 FSMC_NOE RD
	GPIOD->CRL |= 0x00BB0000;   // PD5 FSMC_NWE WR  PD4 5复用推挽
  ...
  FSMC_BTR1  |= 8<<8;         // 数据建立时间DATAST RD-0
  FSMC_BTR1  |= 0x8<<4;       // 地址建立时间ADDSET RD-1
  FSMC_BTR1  |= 0x04<<20;     //分频
  // FSMC_BWTR4[29:28]        // 默认00 使用模式A
	FSMC_BWTR1 |= 8<<8;         // 数据建立时间DATAST WR-0
	FSMC_BWTR1 |= 8<<0;         // 地址建立时间ADDSET WR-
	FSMC_BCR1  |= 1<<0;         // 使能
	FSMC_BWTR1 |= 0x04<<20;
	//屏幕复位
	GPIOE->BSRR |= (1<<1);
